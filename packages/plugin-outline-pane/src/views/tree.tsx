...
import { project, common, event } from '@alilc/lowcode-engine';
import { Node, DragObjectType } from '@alilc/lowcode-designer';
...
const { dragon, activeTracker, isShaken } = designerCabin;
...
export default class TreeView extends Component<{ tree: Tree }> {
  ...
  private onClick = (e: ReactMouseEvent) => {
    ...
    if (isMulti && !node.contains(focusNode) && selection.has(id)) {
      ...
      selection.select(id);
      const selectedNode = project.currentDocument?.selection?.getNodes()?.[0];
