...
import { config, common, event } from '@alilc/lowcode-engine';
...
const { observer, Tip, Title } = editorCabin;
...
export default class TreeTitle extends Component<{
  ...
  render() {
    ...
    const { node } = treeNode;
    const { isParental: isNodeParent, isContainer } = node;
    let style: any;
    if (isCNode) {
    ...
    return (
      ...
      >
        ...
        {config.get('enableCanvasLock', false) && isContainer && isCNode && isNodeParent && <LockBtn treeNode={treeNode} />}
      </div>
    );
  }
}
