...
import { Node, DocumentModel } from '@alilc/lowcode-shell';
import { LocationChildrenDetail } from '@alilc/lowcode-designer';
import { Tree } from './tree';

const { editorCabin, designerCabin } = common;
const { computed, obx, intl, makeObservable, action } = editorCabin;
const { isLocationChildrenDetail } = designerCabin;
...
export default class TreeNode {
  ...
  @computed get detecting() {
    return project.currentDocument?.detecting.current === this.node;
  }
   ...
   */
  isContainer(): boolean {
    return typeof this.node.isContainer === 'function' ? this.node.isContainer() : this.node.isContainer;
  }
