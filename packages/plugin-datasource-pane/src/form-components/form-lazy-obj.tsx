...
import { createSchemaField, useForm,
  useField,
  useFieldSchema,
  observer,
  ISchema,
...
export const FormLazyObj = observer(
  (props: FormLazyObjProps) => {
    ...
    const properties = useMemo(() => {
      return Object.keys(schema.properties).filter((i) => selectedProperties.indexOf(i) === -1).map(
        (propertyKey) => ({
    ...
    const addition = useMemo(() => {
      ...
      if (properties.length === 1) {
        return <Button onClick={handleAdd.bind(this, properties[0].value)}>{addText}</Button>;
      }
      return (
              ...
              (acc, cur) => {
                acc[cur] = {
                  ...
                  'x-decorator-props': {
                    ...
                    addonAfter: <RemoveBtn propertyKey={cur} onClick={handleRemove} />,
                  },
                };
                return acc;
              }, {},
    ...
    return (
      <div
        className={generateClassName('form-lazy-obj')}
      >
        {addition}
        {content}
      </div>
